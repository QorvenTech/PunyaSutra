...
function normalizeSteps(booking) {
  const fallback = [
    ...
    { key: 'puja_scheduled', label: 'Puja Scheduled', status: booking.date ? 'active' : 'pending', message: booking.date ? `Puja is scheduled for ${booking.date}.` : 'Puja schedule will be updated soon.' },
...
function renderTracking(booking) {
  ...
  $('trackResult').innerHTML = `
    <section class="panel trackTimelineCard">
      <div class="trackHeader"><div><p class="eyebrow">Booking Reference ID</p><h2>${safeText(orderId)}</h2></div><span class="badge">${safeText(booking.status || 'In Progress')}</span></div>
      ...
      <div class="timeline">
        ${steps.map((step, index) => `
          <article class="timelineStep ${safeText(step.status)}">
            ...
            <div><strong>${safeText(step.label || step.title || 'Tracking Update')}</strong><p>${safeText(step.message || step.description || 'Update will be shared soon.')}</p>${step.updatedAt || step.time ? `<small>${safeText(dateText(step.updatedAt || step.time))}</small>` : ''}</div>
          </article>
        `).join('')}
      </div>
      <div class="trackNote">You will receive photos and video after the puja is completed.</div>
    </section>
    <aside class="panel trackSummary">
      <h3>Puja Summary</h3>
      <img src="${safeText(imageForBooking(booking))}" alt="${safeText(booking.pujaName || 'Puja booking')}">
      <h4>${safeText(booking.pujaName || 'Puja Booking')}</h4>
      <p class="muted">${safeText(booking.temple || 'Verified Temple')}</p>
      <div class="summaryRows"><span>Date</span><strong>${safeText(booking.date || 'To be scheduled')}</strong><span>Devotee</span><strong>${safeText(booking.userName || booking.devotee?.name || '')}</strong><span>Amount</span><strong>Rs ${Number(booking.amount || 0).toLocaleString('en-IN')}</strong></div>
      <a class="secondary wide" href="./confirmation.html?orderId=${encodeURIComponent(orderId)}">View Details</a>
    </aside>
  `;
}
...
async function loadBooking(orderId) {
  ...
  if (!cleanOrderId) {
    ...
    $('trackMessage').textContent = 'Please enter your booking reference ID.';
    return;
  }
  ...
  if (!snap.exists()) {
    ...
    $('trackMessage').classList.add('error');
    $('trackMessage').textContent = 'No booking found for this reference ID.';
    return;
  }
...
$('trackForm').addEventListener('submit', (event) => {
  ...
  loadBooking(orderId).catch((error) => {
    $('trackMessage').classList.add('error');
    $('trackMessage').textContent = error.message || 'Could not load tracking details.';
  });
});
